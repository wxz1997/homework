package cn.wxz1997.interceptor;
...
public class AuthorizedInterceptor  implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response,
        ...
        //System.out.println("测试拦截器1：" + servletPath );
        /**  判断请求是否需要拦截 */
        for (String s : IGNORE_URI) {
            if (servletPath.contains(s)) {
                //System.out.println("测试拦截器2：" + servletPath + "  s:" + s);
                flag = true;
                break;
            }
        }
        ...
        if (!flag){
            ...
            //System.out.println("测试拦截器3：" + user);
            /** 2.判断用户是否已经登录 */
            if(user == null){
                //System.out.println("测试拦截器4：");
                /** 如果用户没有登录，跳转到登录页面 */
                request.setAttribute("message", "请先登录再访问网站!");
                //此处有bug，页面返回不到login.jsp
