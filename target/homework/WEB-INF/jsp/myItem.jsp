...
<body>
<form action="/homework/item/myItem">
    <div>
...
<table class="table table-striped table-bordered table-hover">
    ...
    <c:forEach items="${myItem}" var="item" >
        <tr>
            ...
            <td>
                <c:choose>
                    <c:when test="${item.success == 0}"><a href="/homework/item/giveup?id=${item.itemId }">放弃</a></c:when>
                    <c:otherwise>goodJob</c:otherwise>
                </c:choose>
            </td>
        </tr>
    </c:forEach>
...
<fkjava:pager pageIndex="${pageModel.pageIndex}"
              ...
              style="digg"
              submitUrl="/homework/item/myItem?pageIndex={0}"/>
